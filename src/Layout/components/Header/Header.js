...
function Header() {
    ...
    const fetchUserInfo = async () => {
        ...
        if (token) {
            try {
                const response = await axios.get('https://cnpmmnhom14.onrender.com/api/accounts/user', {
                    headers: {
    ...
    return (
        <header>
            <div className={cx('row', 'header__top')}>
                <div className={cx('container')} style={{ display: 'flex', justifyContent: 'space-between' }}>
