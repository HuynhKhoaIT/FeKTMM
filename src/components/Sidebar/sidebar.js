...
function Sidebar() {
  ...
  useEffect(() => {
    const fetchAllCagtegories = async () => {
      try {
        const response = await fetch(
          "https://cnpmmnhom14.onrender.com/api/categories/"
        );
        if (!response.ok) {
          throw new Error("Request failed");
        }
      ...
      <div className={cx("side-bar-wrapper")}>
        <nav class="category">
          <h3 className={cx("category__heading")}>
            <i className={cx("heading__icon fa-solid fa-list")}></i>Danh mục
          </h3>
          <ul className={cx("category-list")}>
            {categories.map((category) => (
              <li className={cx("category-item")}>
                <Link
                  key={category._id}
                  to={`/search?keyword=${category._name}`}
                  state={{ keyId: `${category._id}` }}
                  className={cx("category-item__link")}
                >
                  {capitalizeFirstLetter(category._name)}
                </Link>
              </li>
            ))}
          </ul>
