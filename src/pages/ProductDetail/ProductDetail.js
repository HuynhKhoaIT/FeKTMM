...
function ProductDetail({ productId }) {
  ...
  useEffect(() => {
    const updateClickCount = async () => {
      try {
        const response = await fetch(
          `https://cnpmmnhom14.onrender.com/api/products/update-click-count?pId=${pId}`,
          {
            method: "PUT",
            headers: {
        ...
        } else console.log("Cập nhật clickcount thành công!");
      } catch (error) {
        console.log("Cập nhật Clickcount thất bại!");
      }
    };
  ...
  useEffect(() => {
    const fetchProductComments = async () => {
      try {
        const response = await fetch(
          `https://cnpmmnhom14.onrender.com/api/reviews/${pId}`
        );
        if (!response.ok) {
          throw new Error("Request failed");
        }
  ...
  useEffect(() => {
    const fetchRelatedProducts = async () => {
      try {
        const response = await fetch(
          `https://cnpmmnhom14.onrender.com/api/products/related_products/${productDetails._categoryId}`
        );
        if (!response.ok) {
          throw new Error("Request failed");
        }
    ...
    return (
      <div className={cx("wrapper")}>
        ...
        <Container className={cx("container")}>
          ...
          <ProductSlider
            dataDetail={relatedProducts}
            row={1}
            styles={{ padding: "0 !importan" }}
          />
        </Container>
      </div>
    );
