  ...
  return (
    <div className={cx("pImages-wrapper")}>
      <div className={cx("main-pImages-wrapper")}>
        ...
        <img
          className={cx("main-pImage")}
          alt="Ảnh sản phẩm"
          src={pMainImg ? pMainImg : images?.[0]}
        ></img>
      </div>
        ...
        {images?.map((image, index) => {
          if (image === pMainImg) {
            return (
              <img
                key={index}
                className={cx("sub-pImage", "current-pImg")}
                src={image}
                alt="Ảnh sản phẩm"
                onMouseOver={() => handleChangeMainImg(index)}
              ></img>
            );
          } else
            return (
